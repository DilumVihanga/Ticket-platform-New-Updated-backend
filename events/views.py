...
@api_view(['GET'])
def getPurchasebyEvent(request , event_name):
    purchases = TicketPurchase.objects.filter(event_name=event_name)
    serializer = TicketPurchaseSerializer(purchases, many=True)
    return Response(serializer.data)